...
describe('저널=진실원천 — DB 유실에도 대화 기록이 살아남는다', () => {
  ...
  it('addWorkerMessage(워커 채팅)도 저널로 복원된다', () => {
    ...
    expect(listNaviMessages('proj-x').some((m) => m.content === '워커야 빌드해줘')).toBe(true)
  })
  ...
  it('reconcile은 멱등 — 두 번 돌려도 메시지가 중복되지 않는다', () => {
    ...
    const hits = listMessages('manager').filter((m) => m.content === '딱 한 번')
  ...
  it('deleteConversation은 톰스톤으로 영속 — DB가 통째로 날아가도 재부팅 reconcile이 되살리지 않는다', () => {
    ...
    expect(listNaviMessages('proj-del').some((m) => m.content === '지워질 메시지')).toBe(false)

    nukeDb()
    initStore() // 저널엔 지워진 대화의 conv/msg 엔트리 + del 톰스톤이 함께 있다

    expect(listNaviMessages('proj-del').some((m) => m.content === '지워질 메시지')).toBe(false) // 안 되살아남
    expect(listNaviMessages('proj-keep').some((m) => m.content === '남아야 할 메시지')).toBe(true) // 대조군 보존
  })
  ...
  it('reconcile은 톰스톤된 대화를 DB에 행이 남아 있어도 제거한다 (enforce, 멱등)', () => {
    ...
    reconcileFromJournal()
    expect(listNaviMessages('proj-enforce')).toHaveLength(0)
  ...
  it('기존(저널 이전) 메시지도 1회 백필로 저널에 올라가 DB 유실에서 복원된다', () => {
    ...
    nukeDb()
    initStore()
    expect(listMessages('manager').some((m) => m.content === '백필 대상')).toBe(true)
  })
  ...
  it('compactJournal은 삭제대화·톰스톤·설정중복·msg중복을 제거하고 live conv를 원순서로 보존한다 (F1)', () => {
    ...
    const am = listMessages('manager').map((m) => m.content)
    expect(am).toContain('m1')
    expect(am).toContain('m2')
    expect(am).not.toContain('m3')
