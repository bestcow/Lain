...
describe('projectSprite — 키워드 매칭은 이름 기준(경로 prefix 오염 방지)', () => {
  it('tools/ 경로가 gear로 오분류되지 않는다 (asmr→헤드폰·reference→책)', () => {
    // 버그였던 것: id의 "tools"가 "tool"(gear 키워드)에 걸려 tools/* 가 전부 톱니로 보였다.
    expect(spriteFor({ id: 'tools/asmr', name: 'asmr' }).colors['1']).toBe('#5fd0d0') // asmr 테마색
    ...
    expect(spriteFor({ id: 'x/asmr', name: 'asmr' }).colors['1']).toBe('#5fd0d0')
  })
