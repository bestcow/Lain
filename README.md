...
| **교훈 학습** | 자동입니다. 대화·작업에서 배운 규칙이 쌓여 다음 판단에 반영됩니다. 좌측 **학습** 메뉴에서 열람·비활성화할 수 있고, 틀린 행동은 그 자리에서 정정하면 반영됩니다 |
...
Lain·판정·Navi 모델 선택지에 **`local`**(기본 매핑: Qwen3.6-35B-A3B)이 있습니다 — Claude 대신 로컬 [llama.cpp](https://github.com/ggml-org/llama.cpp) `llama-server`(Anthropic Messages API 네이티브)로 라우팅합니다. Python·Ollama·프록시가 필요 없습니다.
...
즉 **8GB급 VRAM에서는 배관은 동작하지만 실용적이지 않습니다.** 대용량 VRAM(24GB+, 모델 전체 GPU 상주)에서는 실용권으로 예상되나 메인테이너 환경에서는 검증할 수 없었습니다 — 이 기능은 **community-supported**이며, 해당 하드웨어에서의 이슈 리포트는 환영하지만 재현 지원이 제한적입니다. 기본값은 Claude 티어이고, 서버가 꺼진 채 `local`을 선택하면 복구 안내가 표시됩니다.
...
**원칙**: 결정론 배관은 코드가, 판단은 Claude가 담당합니다. L0에 LLM 호출을 넣지 않습니다(판단 레이어만 SDK 사용). 설계 전체는 [PLAN.md](PLAN.md), 개발 컨벤션은 [CLAUDE.md](CLAUDE.md), 릴리스 이력은 [CHANGELOG.md](CHANGELOG.md)를 참고하세요.
