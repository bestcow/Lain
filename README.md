...
**내 PC에 상주하며 나에게 길들여지는 개발자용 AI 매니저**입니다 — 대화하고, 여러 프로젝트의 코딩 작업을 지휘·검증·병합하고, 개발 화면이 막힌 순간에만 조언하며, 쓸수록 사용자를 학습합니다. Windows 데스크톱 앱(Electron + Claude Agent SDK)입니다.
...
- **안전장치** — 위험 명령 승인 큐, 시크릿 파일 접근 차단, autonomous 모드 spec-gaming 방어에 더해, 자율로 통과된 결정도 전부 기록되어 승인함 '사후 검토' 탭에서 확인할 수 있습니다. 작업별 토큰 예산으로 폭주도 막습니다
...
1. 실행하면 시작 안내(1회)와 함께 Lain이 인사합니다 — 그냥 대화부터 해보세요.
2. 등록된 프로젝트가 없으면 **Lain이 먼저 프로젝트 폴더 위치를 물어봅니다** — 채팅으로 경로만 알려주면 그 자리에서 등록·스캔됩니다. 수동으로 하려면 프로젝츠 창에서 폴더를 추가하세요. (환경변수 자동 스캔 루트는 아래 [설정](#설정) 참고)
...
- `src/main/` — L0 결정론 코어(`store`·`ipc`·`registry`·`collectors`·`worktree`·`scheduler`) + 판단 레이어(`manager`·`worker`·`navichat`·`orchestrator`, 짧은 판정은 `judge` 러너). store는 `node:sqlite`(네이티브 모듈 아님)
...
**원칙**: 결정론 배관은 코드가, 판단은 Claude가 담당합니다. LLM 호출은 세션 본체(manager·worker·navichat)와 judge 러너에만 허용되고 결정론 배관 모듈엔 넣지 않습니다. 릴리스 이력은 [CHANGELOG.md](CHANGELOG.md)를 참고하세요.
