...
export function ChatPanel({
  ...
  // '새 내용' 경계 — 어깨너머(overlay) 자발 발화는 제외하고 일반 대화의 첫 새 메시지를 기준으로 잡는다
  // (휘발성 관찰이 '새 내용'으로 잡혀 주의를 끌지 않게).
  let boundaryIdx = sessionStart
    ? messages.findIndex((m) => m.createdAt >= sessionStart && m.origin !== 'overlay')
    : 0
      ...
      {messages.map((m, i) => {
        ...
        return (
          <Fragment key={m.id}>
            ...
            <div
              id={`lain-msg-${m.id}`}
              className={`msg msg-${m.role}${m.chapter ? ' msg-chapter' : ''}${activeHitId === m.id ? ' msg-search-active' : ''}${queued ? ' msg-queued' : ''}${m.origin === 'overlay' ? ' msg-proactive' : ''}`}
