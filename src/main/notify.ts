...
//
// ── 통지 기준(새 실패 처리를 붙일 때 이 셋으로 판정한다) ──
// (a) 사용자가 방금 누른 것의 실패는 그 자리(버튼·행·모달)에 표시한다 — notifyUser 토스트 금지.
// (b) 백그라운드 작업이 사람 개입 없이는 영영 회복 못 하는 상태일 때만 notifyUser.
// (c) 재시도로 복구되는 일시 실패는 상태 필드에만 적되, 그 필드가 UI 어딘가에 반드시 보여야 한다
//     (로그 파일 단독 금지 — 사용자가 열어보지 않는 곳은 '기록됨'이지 '보고됨'이 아니다).
