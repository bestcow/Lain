...
import { thinkingOption } from './agentopts'
...
- **작업 중 판단이 필요한 모호함이 생기면 mcp__lain__ask_manager 도구로 질문해라** — 답을 받아 그 자리에서 이어갈 수 있다. 사소한 재량은 묻지 말고 보수적 기본값으로 진행. 비슷한 작업을 전에 어떻게 처리했는지 궁금하면 mcp__lain__search_history로 과거 기록을 먼저 검색해라.${autonomousNote}${workspaceSnapshot(task)}${lessonsBlock(task, countInject)}
  ...
  try {
    ...
    const stream = query({
      ...
      options: {
        ...
        maxTurns: 60,
        model: modelId(opts.modelOverride ?? getSettings().naviModel), // §9b 티어링
