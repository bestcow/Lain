...
import { buildDigest, sendToManager } from './manager'
...
async function refreshBriefing(): Promise<void> {
  const now = Date.now()
  if (now - lastBriefAt < BRIEF_MIN_MS) return
  lastBriefAt = now
...
\`\`\``,
      options: {
        ...
        allowedTools: [],
        maxTurns: 2,
        model: modelId(getSettings().judgeModel), // §9b — 짧은 판정류
        abortController: ac,
        executable: 'node',
...
export async function runScanOnce(): Promise<void> {
  ...
  try {
    ...
    for (const p of listProjects().filter((x) => x.enabled)) {
      if (!before.get(p.id) && p.status?.hasTaskMd) {
