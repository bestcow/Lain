...
- **사분면 표 — 되돌림 가능성 × 확신도 → 개입 단계 (구현 매핑)**: 되돌림 가능성은 `worker.ts` RISKY의 `reversible` 필드(선언) + `classifyDivergence`(명령 단위 판정), 확신도는 프로젝트 실적(`store.ts` loopStats/promotionStats — 완료 실적·최근 rework/심사 미통과)이다. 확신도는 명령 단위가 아니라 **작업 단위**로 소비된다(모드 판정·심사 강도).

  | | 확신도 높음 (실적 충분·사고 없음) | 확신도 낮음 (첫 작업·최근 사고) |
  | --- | --- | --- |
  | **되돌릴 수 있음** | **auto = review-after** — 자율 결정 + `state='auto'` 기록 → 승인함 '사후 검토' 탭에서 사람이 나중에 '확인' | **approve-before 쪽으로 수렴** — 실적 게이트(완료 done≥3 미달 시 자동판정 autonomous 불허 → interactive 승인 큐) + 심사 상향(standard→adversarial) |
  | **되돌릴 수 없음** | **approve-before** — escalate → 승인 큐(§9-4) | **approve-before** — escalate → 승인 큐 + 심사 상향 |

  - 레인에는 무기록 순수 auto가 없다: 자율 결정은 전부 `state='auto'` 행으로 남아 사후 검토 탭에 나타난다(glass-box — auto와 review-after는 연속체이고, 확신도가 낮을수록 사후 검증이 두꺼워진다).
  - **delay 단계(자리 비운 동안 보류했다 몰아서 결재)는 의도적 미구현** — 레인은 단일 사용자 로컬 도구라 승인 큐 대기가 곧 delay와 동치이고, 별도 배치 단계는 복잡도만 더한다.
