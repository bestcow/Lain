...
- 상태: **Phase 0 구현 완료** (2026-06-11, 스캔·수집·대시보드·관리자 채팅 검증됨) — Phase 1 대기
- 최종 수정: 2026-06-12 (§21 Navi 실행 모드 — autonomous/glass-box 편입)
- 위치(배치): **`C:\dev\lain` (확정)**
...
여러 프로젝트(blog, solid, security, main, cowtest, titanic, cow_games, mini_projects …)를 동시에 개발 중이며, 각 현황 파악과 작업 분배가 1인의 인지 부하를 넘는다.
...
- "titanic Navi한테 왜 그 방식 골랐는지 물어봐" → 해당 Navi 세션 `resume`해 질문·답 중계
...
│ ┌blog────┐ ┌solid───┐ ┌titanic─┐ ┌security─┐ ┌main────┐ ┌cowtest─┐     │
│ │  ☻ ✓   │ │  ☻ ✗   │ │  ☺ ⌨   │ │  ☻ ?    │ │  ☻ ▮   │ │  ☻ z   │     │
│ │ blog   │ │ solid  │ │titanic │ │security │ │ main   │ │cowtest │     │
│ │변경없음 │ │테스트  │ │OAuth   │ │콜백 URL │ │push    │ │대기 중 │     │
│ │        │ │회귀수정 │ │리팩터링 │ │확인대기 │ │승인대기 │ │        │     │
│ └────────┘ └────────┘ └────────┘ └────────┘ └────────┘ └────────┘     │
│ ┌cowbat──┐ ┌tarot───┐ ... 더 많아지면 다음 줄로 자동 wrap → 창 높이↑   │
├ INPUT ──────────────────────────────────────────────────────────────────┤
│ user@maestro:~$ ▋                                          [▶착수][⏸]   │
└──────────────────────────────────────────────────────────────────────────┘
...
- **입력(하단) + 대상 선택:** 프롬프트 한 줄 + 착수/전체중지. **대상 칩**으로 `[관리자]`(기본)·`[titanic]`(1명)·`[3명]`(다수)·`[전체]` 전환(§5.6). Navi 대상이면 그 Claude Code에 바로 입력하는 것과 동일.
...
- **채팅:** 입력은 프롬프트 `>` 스타일, 너=`user@maestro:~$`, 관리자=`manager>`, Navi 인용=`[titanic]>` 접두. Navi 활동 로그는 콘솔 스트림처럼 한 줄씩 흐름.
...
- **Godot(cow_games)**: `npm test` 없음 → verify_cmd를 Godot 헤드리스 또는 "검증 없음"으로. 스택별 검증 어댑터.
...
### 21.7 빌드 기반 결정 — 보류 (§18 실측 후 확정)
원 계획은 raw API 얇은 루프(SDK 포기)를 주장. lain interactive Navi는 SDK. **둘은 같은 Navi에 동시 적용 불가** → autonomous 착수 시점에 결정한다.
- 판단 기준은 "남의 거냐"가 아니라 **lock-in(갈아끼울 수 있나)**. Navi 실행부를 **인터페이스 뒤에** 두면 밑이 raw든 SDK든 구현 디테일이고 교체 가능.
- 후보 A: **SDK 재사용**(interactive와 코어 공유, hands-off는 `permissionMode`로) — 통합 비용 낮음. / 후보 B: **raw 루프**(hardened 컨텍스트·권한 직접 짐) — 차별점 통제력 높음, 유지보수 직접. → §18 실측 결과로 결정.
...
- **autonomous Navi = Phase 3 이후의 별도 트랙.** 전제: ① interactive 검증 완료 ② §18 SDK 실측 완료 ③ 이벤트 소싱 리팩터(§21.4) 결정. 빌드 순서: 얇은 수직 슬라이스(red→green 자동채점 task) → spec-gaming 방어 → divergence 정책 → elicitation 엔진 + fuzzy task.
...
> 모델 가중치가 아니라 **검증된 경험을 교훈으로 쌓고 다음 작업에 검색해 주입**해서 점점 똑똑해진다. (가중치 학습은 후속.) §21.4 glass-box "성장이 보인다"의 최소 구현.

### 22.1 폐루프
작업 → **verify pass로 review 도달** → 회고(judge가 재사용 교훈 추출) → `lessons` 저장 → 다음 Navi 스폰 시 같은 프로젝트 교훈을 프롬프트에 주입(retrieval) → 행동 개선. E2E로 전 구간 검증(seed 교훈 "결과는 dist/에" → TASK.md에 없는데도 Navi가 dist/에 생성, reuse_count++).

### 22.2 안전장치 (핵심)
- **verify pass한 작업의 교훈만 신뢰.** 자동 채점 안 되면(verify_cmd 없음) 회고 건너뜀 — 틀린 교훈 누적이 자기개선을 망치므로. autonomous(테스트=판사)와 가장 잘 맞음.
...
- retrieval: `worker.lessonsBlock()` — fresh 스폰 시 프로젝트+global 교훈 top-K 주입, reuse_count++.
- UI: 🧠 LESSONS 패널(누적 건수·재사용 횟수·scope 뱃지) = 성장 가시화.

### 22.4 다음 단계 (미구현)
- **임베딩 검색**: 지금은 프로젝트 매칭+재사용순 top-K. 8GB 로컬 임베딩 모델로 의미 검색 → 관련도 향상.
- **교훈 정제**: 중복 병합·오래된/안 쓰이는 교훈 폐기(reuse_count 0 장기 방치 정리), lain 자체 메모리 consolidate처럼.
...
### 23.1 동작
`bench/<task>/`(TASK.md + 검증 + 선택적 lessons.json) 묶음을 **교훈 off/on 두 조건**으로 격리 실행 → `성공률·1회통과율·평균 턴·평균 비용` 비교. 각 task는 임시 git repo로 materialize → 임시 프로젝트 등록 → `startTask(skipClarify)` → 종결 폴링 → 결과 수집 → 완전 정리(worktree·레지스트리·repo).
...
**해석(중요)**: Navi가 영리해서 검증 명령(판사)을 읽고 역공학 → 교훈 없이도 성공. 하지만 교훈은 **시행착오를 건너뛰게** 해 턴 12%·비용 7% 절감(dist-convention 10→8턴). → "자기개선은 성공률이 아니라 효율을 올린다"를 측정으로 입증. **검증이 자기설명적이면 교훈 효과가 작다**는 것도 평가 하네스가 밝힌 통찰.
...
- **검증으로 역공학 안 되는 task**: 런타임 절차 함정(빌드 선행 등) — 교훈 효과가 크게 드러나는 시나리오로 자기개선 가치를 선명히.
- **회귀 감지**: 자기개선이 지표를 **악화**시키는 경우(틀린 교훈 누적) 자동 경보 — 평가 하네스를 CI처럼 주기 실행.
- **교훈 품질 루프 연결(§22 + Hermes curator 차용)**: 평가에서 "도움 안 된 교훈"을 폐기·정제하는 피드백.
...
- **Phase 3 — 교훈 품질 루프(Curator)** *(lifecycle·telemetry·bench 선행)*: **idle 트리거 judge curator**(중복병합·미사용 soft-archive, 하드삭제 금지) `M·high` ← §22.4 핵심 격차 · 사용 중 교훈 자기교정 patch-on-use(Navi는 신고만, judge가 verify-pass 게이트 뒤 retire/patch) `M·high` · 출처 게이트 `origin=agent|user`(user 교훈 불가침, curator PR에 동봉) `S`.
- **Phase 4 — 지능 확장** *(측정 후 opt-in)* ⚠️ verify 밖 학습이라 §22.2("verify pass한 교훈만 신뢰")와 긴장 — **회귀감지(Phase0) 없이 도입 금지**: 사용자-교정 신호 기반 background review `M` · judge cadence 사용자 모델(개인화) `M·high` · 하위디렉터리 규약 progressive 주입(SDK PostToolUse 실측 후) `M` · 교훈→실행가능 절차 결정화(curator 선행) `M`.
...
- ✅ **Phase 0**: FTS5/trigram 실측(§24.1), bench `extraResources` no-op 수정, 회귀 감지(`bench.ts detectRegression` — 교훈 ON이 성공률/효율 악화 시 경보, OS 알림).
- ✅ **Phase 1**: 비밀파일 데노리스트(`safety.ts`), 교훈 수명주기+telemetry+origin(`applyLessonLifecycle`·scheduler idle 틱), 워크스페이스 스냅샷, verify 실패 분류(`classifyVerifyFailure`), temporal anchoring(매니저 현재시각 헤더·Navi resume STALE), spec-gaming 강화(autonomous verify pass 후 `changedFiles`로 테스트 파일 변경 사후검증 → blocked).
- ✅ **Phase 2**: 도구-루프 가드(`TOOL_LOOP_BLOCK` sha256 시그니처), 교차세션 검색 도구(`searchHistory` — LIKE·project 스코프·worker/manager `search_history` MCP), 교훈 콘텐츠-인지 랭킹(`lessonsForProject(…, task.content)` — 작업과 키워드 겹치는 교훈 우선). FTS5 trigram은 한국어 2글자 매칭 실패 실측 → 소형 테이블엔 LIKE/JS 스코어링 채택, 대량화 시 FTS 도입.
- ✅ **Phase 3 (Curator 완성)**:
  - patch-on-use 신고 채널(`flagLesson` + Navi `flag_lesson` MCP — 틀린 교훈 신고 시 즉시 soft-archive, judge LLM 불필요).
  - **idle judge consolidate**(`scheduler.consolidateLessons` — 중복 교훈을 semantic 병합·umbrella 삽입·소스 soft-archive). **opt-in `lessonCurator` 기본 off**, autoPriority 격리 패턴(해시가드·60s abort·작업중 skip·한 틱 최대 3병합·그룹당 2건↑). L0의 두 번째이자 마지막 LLM 예외 — 격리를 코드 불변식으로 강제(§24.4).
  - **provenance 게이트 실효**: curator·flag·lifecycle 전부 `origin='agent'`·비핀만 건드림 → user/pinned 교훈 불가침 보장(단 user 교훈 *입력* UI는 아직 없음).
  - → **교훈 폐루프 완성**: 생성(reflect) → 콘텐츠랭킹 → 노화(lifecycle) → 신고회수(flag) → 정비(curator 병합) → 측정(bench+회귀).
- 🔜 **남음**: 런타임-함정 fixture(§23.4), user 교훈 입력 UI(provenance 활용), Phase 4 지능확장(background review·사용자 모델·결정화). **전부 미배포 — `npm run deploy` 일괄 필요.**
